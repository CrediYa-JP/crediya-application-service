package co.com.crediya.app.model.exception.constants;



public final class BusinessErrorMessages {

    public static final String INVALID_LOAN_TYPE_MSG = "Selected loan type does not exist";
    public static final String USER_NOT_FOUND_MSG = "User not found in authentication system";
    public static final String UNAUTHORIZED_OPERATION_MSG = "User is not authorized to perform this operation";

    private BusinessErrorMessages() {}
}